...
class QuadrotorDynamicObstacleNCBF:
    ...
    def load_value_net(self, ckpt_path: pathlib.Path | None = None):
        ...
        if ckpt_path is None:
            ckpt_path = "/Users/myles/Programming/cornell/cs5757/DONSVIMPC/runs/offline_drone_do/utku/00175000/default"
    ...
    def get_norm_obs(self, state: jnp.ndarray, obstacles):
        assert state.shape == (6,)
        obs = state_to_obs_drone(state, obstacles)
        norm_obs = self.obs_norm.normalize(obs)
        return norm_obs

    def get_h_vector(self, state: jnp.ndarray, obstacles: jnp.ndarray):
        assert state.shape == (6,)
        h_h = get_h_vector_drone(state, obstacles)
        return h_h
    ...
    def get_h(
        ...
        norm_obs = self.get_norm_obs(state, obstacles)

        # 2: Get the base h value.
        h_h = self.get_h_vector(state, obstacles)
...
class QuadrotorMPPIDynamicObstacleNCBFCostEvaluator(QuadraticCostEvaluator):
    def __init__(
        self,
        goal_checker=None,
        ...
        self.ncbf = QuadrotorDynamicObstacleNCBF()
        self.cbf_alpha = 0.9
    ...
    def evaluate_cost(
        ...
        obstacles = self.collision_checker.get_obstacle_matrix()

        # 4: CBF cost.
        get_h = ft.partial(self.ncbf.get_h, ncbf_weights=ncbf_weights)
        bh_Vh = jax.vmap(get_h, in_axes=(0, None))(b_state, obstacles)
        h_Vh_now, h_Vh_next = bh_Vh[0], bh_Vh[1]

        h_h_now = self.ncbf.get_h_vector(state_cur, obstacles)
        h_h_next = self.ncbf.get_h_vector(state_next, obstacles)
    ...
    def evaluate_cbf_cost(self, state_cur, dynamics, state_next):
        ...
        def danger_index(x):
            obstacles = self.collision_checker.get_obstacle_matrix()
            h_h = get_h_vector_drone(x, obstacles)
            return h_h
