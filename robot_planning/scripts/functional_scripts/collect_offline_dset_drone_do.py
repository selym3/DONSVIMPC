...
def main():
    ...
    for traj_idx in tqdm.trange(n_trajs_collect):
        ...
        TN_o = logger.obstacle_states

        if onp.isnan(T_x).any() or onp.isnan(TN_o).any():
    ...
    data_dir = pathlib.Path(__file__).parent.parent.parent / "data_drone_do"
    data_dir.mkdir(exist_ok=True, parents=True)

    pkl_path = data_dir / "raw_data.pkl"
    with open(pkl_path, "wb") as f:
        pickle.dump((bT_x, bTN_o), f, pickle.HIGHEST_PROTOCOL)
